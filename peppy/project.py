...
class Project(PathExAttMap):
    ...
    def __init__(
        ...
        self.sst_index = (
            ...
            or (
                [sst_index] if (sst_index := getattr(self, "sst_index", None)) else None
            )
    ...
    def subsample_table(self):
        ...
        """
        if not (subsample_dataframes_array := self[SUBSAMPLE_DF_KEY]):
            return None

        subsample_dataframes_array = make_list(subsample_dataframes_array, pd.DataFrame)
