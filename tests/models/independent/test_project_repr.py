...
def check_samples_repr(p):
    ...
    exp_names = all_names if len(all_names) <= MAX_PROJECT_SAMPLES_REPR \
        else all_names[:MAX_PROJECT_SAMPLES_REPR]
    if exp_names != obs_names:
        return "Expected sample names {} but found {} in repr".\
            format(all_names, obs_names)
...
def check_sections_repr(p):
    ...
    if len(rel_lines) != 1:
        return "Project should have exactly one line for sections but has {}: " \
               "{}".format(len(rel_lines), rel_lines)
    text = rel_lines[0]
    names = text.split(": ")[1].strip().split(", ")
    data = [(n, getattr(p, n)) for n in names]
    bad_names = [n for n, obj in data if n.startswith("_")
                 or isinstance(obj, property) or callable(obj)]
    if bad_names:
...
def check_subs_repr(p):
    ...
    if subs:
        ...
        if not rel_lines:
            return "No subproject lines in project repr, but project has " \
        ...
        if missing:
            return "Missing subproject names from relevant repr line ({}): " \
                   "{}".format(text, missing)
    elif rel_lines:
        return "Project lacks subprojects but has subproject lines in repr: {}".\
            format(rel_lines)
...
def check_line_count(p):
    ...
    if exp_num_lines != len(obs_lines):
        return "Expected {} repr lines but got {}: {}".\
            format(exp_num_lines, len(obs_lines), obs_lines)
...
class ProjectReprTests:
    ...
    @pytest.mark.parametrize(
        "invalidate", [check_line_count, check_subs_repr,
                       check_sections_repr, check_samples_repr])
    def test_project_repr(self, example_peps, invalidate):
        assert len(example_peps) > 0
        print("Checking {} projects: {}".format(
        ...
        for n, p in example_peps.items():
            ...
            if err:
                fails.append((n, err))
        if fails:
            pytest.fail("{} projects failed validation: {}".
...
def _determine_project_paths(data_root):
    ...
    assert os.path.isdir(data_root), \
        "Data root path isn't a folder: {}".format(data_root)
